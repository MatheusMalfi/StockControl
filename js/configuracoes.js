...
function getCurrentOrganizationStorageKey() {
  ...
  return organization_id
    ? `sc_organizacao_${organization_id}`
    : "sc_organizacao";
}
...
function getCurrentPreferencesStorageKey() {
  const { organization_id } = _getOrgData();
  return organization_id
    ? `sc_preferencias_${organization_id}`
    : "sc_preferencias";
}
...
function carregarDados() {
  ...
  _cfgApi("GET", `/api/configuracoes${_oid ? "?organization_id=" + _oid : ""}`)
    .then((data) => {
      ...
      if (data.categorias)
        localStorage.setItem("sc_categorias", JSON.stringify(data.categorias));
    })
    .catch(() => {});
  ...
  seed(getCurrentUserStorageKey());
  seed("sc_usuarios");
  seed(getCurrentOrganizationStorageKey());
  seed(getCurrentPreferencesStorageKey());
  seed("sc_notif_rules");
  seed("log_acessos");
  ...
  if (!catRaw) {
    salvarNoLocalStorage("sc_categorias");
  } else {
    try {
      const parsed = JSON.parse(catRaw);
      // Old format: { categorias: ['str', ...], marcas: ['str', ...] }
      if (parsed.categorias && typeof parsed.categorias[0] === "string") {
        parsed.categorias = parsed.categorias.map((nome, i) => ({
          id: gerarId(),
          nome,
          cor: "#3b82f6",
          ordem: i,
        }));
        parsed.marcas = (parsed.marcas || []).map((nome) =>
          typeof nome === "string" ? { id: gerarId(), nome } : nome,
        );
  ...
  if (!locRaw) {
    salvarNoLocalStorage("sc_localizacoes");
  } else {
    try {
      ...
      if (
        ...
        const migrated = parsed.map((nome, i) => ({
          id: gerarId(),
          nome,
          tipo: "outro",
          descricao: "",
...
function validarCNPJ(cnpj) {
  ...
  return (
    d[12] === mod(calc(d.slice(0, 12), p1)) &&
    d[13] === mod(calc(d.slice(0, 13), p2))
  );
}
...
function showToast(msg, tipo = "info") {
  if (typeof SC !== "undefined" && SC.toast) {
    ...
    cont.appendChild(t);
    const hideToast = () => {
      if (!t.parentElement || t.classList.contains("hiding")) return;
...
function initConfiguracoes() {
  ...
  document
    .getElementById("modalUsuarioSenha")
  ...
  ["localStorage", "sessionStorage"].forEach((store) => {
    try {
      ...
      if (raw) {
        ...
        s.name = nome;
        window[store].setItem("sc_user", JSON.stringify(s));
      }
    } catch {}
  });

  const { id: _uid, organization_id: _poid } = _getOrgData();
  _cfgApi("PUT", "/api/configuracoes/perfil", {
    nome,
    email: u.email,
    user_id: _uid,
    organization_id: _poid,
  }).catch(() => {});
  registrarLog("Perfil atualizado", "perfil", nome, "");
  atualizarAvatarHeader();
  _perfilSnapshot = { nome, email: u.email, avatar: u.avatar || null };
  detectarMudancasPerfil();
  showToast("Perfil atualizado!", "success");
  // Notificação visual igual ao botão 'Salvar Preferências'
...
function uploadFotoPerfil(file) {
  ...
  if (file.size > 2 * 1024 * 1024) {
    showToast("Imagem deve ter menos de 2 MB.", "error");
    return Promise.reject();
  }
...
function detectarMudancasPerfil() {
  ...
  const currentAvatar = _pendingAvatarRemoved
    ? null
    : _pendingAvatarDataUrl || carregarUsuarioLocal({}).avatar || null;
  const avatarMudou = currentAvatar !== _perfilSnapshot.avatar;
  const mudou = nomeMudou || avatarMudou;
  btn.disabled = !mudou;
  if (!mudou) btn.classList.remove("btn-success");
...
function salvarPreferenciasExibicao() {
  const prefs = {
    ...
    formatoData:
      document.getElementById("prefFormatoData")?.value || "DD/MM/AAAA",
  };
  salvarNoLocalStorage(getCurrentPreferencesStorageKey(), prefs);
  const { organization_id: _preoid } = _getOrgData();
  _cfgApi("PUT", "/api/configuracoes/preferencias", {
    ...prefs,
    organization_id: _preoid,
  }).catch(() => {});
...
function salvarOrganizacao() {
  ...
  _cfgApi("PUT", "/api/configuracoes/organizacao", {
    ...org,
    organization_id: _ooid,
  }).catch(() => {});
...
function uploadLogoOrganizacao(file) {
  ...
  if (file.size > 1024 * 1024) {
    showToast("Logo deve ter menos de 1 MB.", "error");
    return;
  }
  const reader = new FileReader();
  reader.onload = (e) => {
...
function salvarUsuario() {
  ...
  if (id) {
    ...
    _cfgApi("POST", "/api/usuarios", {
      nome,
      email,
      cargo,
      role,
      senha,
      organization_id: _uoid,
    }).catch(() => {});
  }
...
function excluirUsuario(id) {
  ...
  if (id === atual.id) {
    showToast("Você não pode excluir sua própria conta aqui.", "error");
    return;
  }
  const u = carregarDoLocalStorage("sc_usuarios", []).find((x) => x.id === id);
  confirmarAcao(
    "Excluir usuário",
    `Excluir ${u?.nome || "este usuário"}? Esta ação não pode ser desfeita.`,
    () => {
      const lista = carregarDoLocalStorage("sc_usuarios", []).filter(
        (x) => x.id !== id,
      );
      salvarNoLocalStorage("sc_usuarios", lista);
      carregarUsuarios();
...
function carregarCategorias() {
  const data = carregarDoLocalStorage("sc_categorias");
  renderListaCategorias(data.categorias || []);
  const { organization_id } = _getOrgData();
  if (!organization_id) return;
  _cfgApi(
    "GET",
    `/api/configuracoes/categorias?organization_id=${organization_id}`,
  )
    .then((res) => {
      if (!res.success) return;
      const stored = carregarDoLocalStorage("sc_categorias");
...
function renderListaCategorias(cats) {
  ...
  if (!cats || !cats.length) {
    cont.innerHTML =
      '<p style="color:var(--color-text-muted);font-size:0.875rem;padding:var(--space-3) 0;">Nenhuma categoria cadastrada.</p>';
    return;
  }
...
function buscarCategorias(termo) {
  ...
  renderListaCategorias(
    t ? cats.filter((c) => c.nome.toLowerCase().includes(t)) : cats,
  );
}
...
function abrirModalNovaCategoria() {
  ...
  selecionarCorCategoria(
    "#3b82f6",
    document.querySelector("#corPickerCategoria .cor-swatch"),
  );
...
function abrirModalEditarCategoria(id) {
  ...
  const sw = document.querySelector(
    `#corPickerCategoria .cor-swatch[data-cor="${CSS.escape(cat.cor || "#3b82f6")}"]`,
  );
...
function salvarCategoria() {
  ...
  registrarLog(
    id ? "Categoria editada" : "Categoria criada",
    "categorias",
    nome,
    "",
  );
...
function excluirCategoria(id) {
  ...
  const warn =
    count > 0
      ? ` (${count} ${count === 1 ? "item usa" : "itens usam"} esta categoria)`
      : "";
...
function selecionarCorCategoria(cor, el) {
  ...
  document
    .querySelectorAll("#corPickerCategoria .cor-swatch")
...
function initDragDropCategorias() {
  ...
  lista.addEventListener("drop", (e) => {
    e.preventDefault();
    const data = carregarDoLocalStorage("sc_categorias");
    const novosIds = [...lista.querySelectorAll(".categoria-item")].map(
      (el) => el.dataset.id,
    );
    data.categorias = novosIds
      .map((id, i) => {
        const cat = (data.categorias || []).find((c) => c.id === id);
...
function renderListaMarcas(marcas) {
  ...
  cont.querySelectorAll("[data-marca-idx]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const d = carregarDoLocalStorage("sc_categorias");
...
function adicionarMarca() {
  ...
  if (data.marcas.some((m) => (m.nome || m) === val)) {
    showToast("Marca já existe.", "warning");
    return;
  }
...
function _syncLocToApi(lista) {
  const { organization_id } = _getOrgData();
  if (!organization_id) return;
  _cfgApi("PUT", "/api/configuracoes/localizacoes", {
    organization_id,
    localizacoes: lista,
  }).catch(() => {});
}
...
function carregarLocalizacoes() {
  ...
  _cfgApi(
    "GET",
    `/api/configuracoes/localizacoes?organization_id=${organization_id}`,
  )
    .then((res) => {
      if (!res.success) return;
...
function renderListaLocalizacoes(locs) {
  ...
  if (!locs || !locs.length) {
    cont.innerHTML =
      '<p style="color:var(--color-text-muted);font-size:0.875rem;padding:var(--space-3) 0;">Nenhuma localização cadastrada.</p>';
    return;
  }
  cont.innerHTML = locs
    .map((loc) => {
...
function buscarLocalizacoes(termo) {
  ...
  let lista = carregarDoLocalStorage("sc_localizacoes");
...
function abrirModalNovaLocalizacao() {
  document.getElementById("modalLocalizacaoTitulo").textContent =
    "Nova Localização";
...
function abrirModalEditarLocalizacao(id) {
  const loc = carregarDoLocalStorage("sc_localizacoes", []).find(
    (l) => l.id === id,
  );
  if (!loc) return;
  document.getElementById("modalLocalizacaoTitulo").textContent =
  ...
  document.getElementById("modalLocalizacaoTipo").value =
    loc.tipo || "deposito";
  document.getElementById("modalLocalizacaoCapacidade").value =
    loc.capacidade || "";
  document.getElementById("modalLocalizacaoDescricao").value =
...
function salvarLocalizacao() {
  ...
  const tipo =
    document.getElementById("modalLocalizacaoTipo")?.value || "outro";
  const cap =
    parseInt(document.getElementById("modalLocalizacaoCapacidade")?.value) ||
    null;
  const desc =
    document.getElementById("modalLocalizacaoDescricao")?.value.trim() || "";
  if (!nome) {
    showToast("Nome é obrigatório.", "error");
    return;
  }
  ...
  renderListaLocalizacoes(lista);
  registrarLog(
    id ? "Localização editada" : "Localização criada",
    "localizacoes",
    nome,
    "",
  );
...
function excluirLocalizacao(id) {
  ...
  const warn =
    count > 0
      ? ` (${count} ${count === 1 ? "item está" : "itens estão"} aqui)`
      : "";
  confirmarAcao("Excluir localização", `Excluir "${loc.nome}"?${warn}`, () => {
    const lista = carregarDoLocalStorage("sc_localizacoes", []).filter(
      (l) => l.id !== id,
    );
    salvarNoLocalStorage("sc_localizacoes", lista);
    _syncLocToApi(lista);
...
function carregarPreferenciasNotificacao() {
  ...
  _cfgApi(
    "GET",
    `/api/configuracoes/notificacoes?organization_id=${organization_id}`,
  )
    .then((res) => {
...
function salvarPreferenciasNotificacao() {
  ...
  _cfgApi("PUT", "/api/configuracoes/notificacoes", {
    ...regras,
    organization_id: _noid,
  }).catch(() => {});
  showToast("Preferências salvas!", "success");
}
...
function alterarSenha() {
  ...
  hideErr("errorCurrentPwd");
  hideErr("errorNewPwd");
  hideErr("errorConfirmPwd");

  if (!atual) {
    showErr("errorCurrentPwd", "Senha atual é obrigatória.");
    return;
  }
  ...
  if (!v.len || !v.upper || !v.number || !v.special) {
    showErr(
      "errorNewPwd",
      "A senha deve ter 8+ caracteres, letra maiúscula, número e caractere especial.",
    );
    return;
  }
  if (nova !== confirma) {
    showErr("errorConfirmPwd", "As senhas não coincidem.");
    return;
  }
...
function renderBarraForca(senha) {
  ...
  if (label) {
    label.textContent = pct > 0 ? calcularForcaSenha(senha).nivel : "—";
  }
}
...
function renderLogAcessos(limite) {
  ...
  if (!itens.length) {
    cont.innerHTML =
      '<p style="color:var(--color-text-muted);font-size:0.875rem;">Nenhum acesso registrado.</p>';
    return;
  }
...
function encerrarTodasSessoes() {
  confirmarAcao(
    "Encerrar sessões",
    "Isso encerrará todas as sessões ativas. Continuar?",
    () => {
      const raw = carregarDoLocalStorage("log_acessos", []);
...
function initDangerZone() {
  document
    .getElementById("btnExportarDados")
    ?.addEventListener("click", exportarTodosDados);
}
...
function wireProfileEvents() {
  ...
  document
    ...
    ?.addEventListener("click", removerFotoPerfil);
  document
    .getElementById("saveProfileBtn")
    ?.addEventListener("click", salvarPerfil);
  document
    .getElementById("savePrefsBtn")
    ?.addEventListener("click", salvarPreferenciasExibicao);
}

function wireOrgEvents() {
  document
    .getElementById("saveOrgBtn")
    ?.addEventListener("click", salvarOrganizacao);
  document.getElementById("saveGoalBtn")?.addEventListener("click", () => {
    const org = carregarDoLocalStorage(getCurrentOrganizationStorageKey(), {});
    org.meta = {
      quantidade:
        parseInt(document.getElementById("goalTarget")?.value) || null,
  ...
  document
    .getElementById("orgLogoInput")
    ?.addEventListener("change", function () {
      uploadLogoOrganizacao(this.files[0]);
    });
  document
    .getElementById("btnRemoverLogo")
    ?.addEventListener("click", removerLogoOrganizacao);
}

function wireNotifEvents() {
  document
    .getElementById("saveNotifBtn")
    ?.addEventListener("click", salvarPreferenciasNotificacao);
  document
    .getElementById("notifLowStock")
    ?.addEventListener("change", function () {
...
function wireCategoriasEvents() {
  document
    ...
    ?.addEventListener("click", adicionarMarca);
